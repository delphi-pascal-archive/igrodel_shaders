// small update: *.HWL support added by Georgy Moshkin
...
type
  ...
  TGLBMP = class
    ...
    procedure HWL_SwapRB;
    function LoadHWL(filename: String) : Boolean;
  private
...
procedure LoadTexture(fname:string;var texid:GLUINT);
...
function myext(fname:string):string;
var tmp:string;
begin
 tmp:=upcase(fname[length(fname)-2])+
      upcase(fname[length(fname)-1])+
      upcase(fname[length(fname)]);
 result:=tmp;
end;
...
begin
  ...
  if (myext(filename)='BMP') then begin
    Result := LoadBMP(filename);
    Exit;
  end;

  if (myext(filename)='TGA') then begin
    Result := LoadTGA(filename);
    Exit;
  end;

  if (myext(filename)='HWL') then begin
    Result := LoadHWL(filename);
    Exit;
  end;
end;
...
procedure LoadTexture(fname:string;var texid:GLUINT);
var tempTexture:TGLBMP;
begin

tempTexture:=TGLBMP.Create(fname);
//tempTexture.SetTextureWrap(GL_CLAMP_TO_EDGE,GL_CLAMP_TO_EDGE);
tempTexture.GenTexture;
texid:=tempTexture.TextureID;
tempTexture.ClearMem;

end;


procedure TGLBMP.HWL_SwapRB;
var
  i, x, y : Integer;
  pixel : PGLRGBQUAD;
  shitpixel : PGLRGBQUAD;
//  tmp : Byte;
//  tempRGB:TGLRGBQUAD;
  picbits:PGLRGBQUAD;
...
begin
  ...
  getmem(picbits,width*height*4);

 
   for y := 0 to Height - 1 do begin
    for x := 0 to Width - 1 do begin
    Pixel := Pointer(Cardinal(rgbBits) + (x+1)*4 + Width*4*y - 4);
    ShitPixel := Pointer(Cardinal(picBits) + (x+1)*4 + Width*4*(Height-y-1) - 4);
    ShitPixel^:=Pixel^;
    end;
  end;
  ...
  for x := 0 to Width - 1 do begin
    for y := 0 to Height - 1 do begin
    Pixel := Pointer(Cardinal(rgbBits) + i*4);
    ShitPixel := Pointer(Cardinal(picBits)+i*4);
    Pixel^:=ShitPixel^;
    inc(i);
    end;
  end;
  ...
  freemem(picBits,width*height*4);
...
function TGLBMP.LoadHWL(filename: String) : Boolean;
var hwl_head    : HWL_Header;
    bgrBits8    : PByte;     // Temp image buffer
    source8     : PByte;
    dest : PGLRGBQUAD;  // Pointer to 24bpp image
    pad , x, y : Integer;       // Loop counters
//    f:file;

begin

  ReadHeaderHWL(filename, hwl_head);

  (*
  assignFile(f,'zzz');
  rewrite(f,1);
  blockwrite(f,hwl_head,sizeof(hwl_head));
  closeFile(f);
    *)

  Width := hwl_head.width;
  Height := hwl_head.height;
  ColorDepth := hwl_head.bpp;
  ImageSize := Width*Height*4;
    getmem(rgbBits,ImageSize);

  if hwl_head.bpp=24 then
   begin
    HWL2Mem(hack,hwl_head,filename,true);
    rgbBits:=hack;
    HWL_SwapRB();
   end
    else
